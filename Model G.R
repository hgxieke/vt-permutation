# ============================================================
# Model G Simulation: power under Correlated Errors
# ============================================================

setwd("C:/Users/Ke/OneDrive - Washington University in St. Louis/Desktop/python/Model22/cor_error/ModelG/")
...
generate_data <- function(N, nx, CUT0, beta, sigma, rho, ite_method = "ite2") {
  ...
  mu <- beta[1] +
    beta[2] * exp((X_df$X1 - 0.5)^2 + (X_df$X2 - 0.5)^2) +
    beta[3] * trt * Z1 + beta[4] * trt * Z2
  ...
  for (N in n_vals) {
    for (ite_method_to_use in ite_methods) {
      ...
      for (sim in 1:nrun) {
        ...
        true_groups <- with(original_data, 2 * Z1 + Z2 + 1)
...
write.csv(results_summary, "ModelG.csv", row.names = FALSE)
